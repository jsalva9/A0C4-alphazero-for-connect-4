        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pygame.quit()
                    sys.exit()
                elif self.agent is not None and not is_over and self.board.get_current_player() == 1:
                    time.sleep(0.5)
                    # Agent's turn
            ...
            if is_over:
                text = self.font.render(f"{ {1: 'Red', -1: 'Blue'}.get(board.check_winner())} wins!", True,
                                        self.FONT_COLOR)
    ...
    def draw_board(self, board, turn=True, priors=None):
            ...
            for col in range(self.board.w):
                ...
                pygame.draw.circle(self.screen, self.LINE_COLOR,
                                   (col * self.CELL_SIZE + self.CELL_SIZE // 2,
                                    row * self.CELL_SIZE + self.CELL_SIZE // 2), self.RADIUS, 0)

        for row in range(self.board.h):
            for col in range(self.board.w):
                if self.board.state_representation[col][row] == 1:
                    pygame.draw.circle(self.screen, self.PLAYER_COLORS[0],
                                       (col * self.CELL_SIZE + self.CELL_SIZE // 2,
                                        self.HEIGHT - (row + 3) * self.CELL_SIZE + self.CELL_SIZE // 2), self.RADIUS, 0)
                elif self.board.state_representation[col][row] == -1:
                    pygame.draw.circle(self.screen, self.PLAYER_COLORS[1],
                                       (col * self.CELL_SIZE + self.CELL_SIZE // 2,
                                        self.HEIGHT - (row + 3) * self.CELL_SIZE + self.CELL_SIZE // 2), self.RADIUS, 0)
        ...
        if turn:
            text = self.font.render(
                f"Turn of { {0: 'red', 1: 'blue'}.get(board.get_current_player())}", True, self.FONT_COLOR)
            text_rect = text.get_rect(center=(self.WIDTH // 2, self.HEIGHT -  self.CELL_SIZE // 2))
            self.screen.blit(text, text_rect)
